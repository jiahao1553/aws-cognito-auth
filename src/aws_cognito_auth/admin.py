...
@click.option("--lambda-code", type=click.Path(exists=True), help="Path to Lambda function code (uses built-in if not provided)")
def deploy(region, access_key_id, secret_access_key, create_user, lambda_code):
    ...
    try:
        ...
        if access_key_id and secret_access_key:
            ...
            print("\nExample:")
            print("   python admin.py lambda deploy --access-key-id AKIA... --secret-access-key ... --lambda-code lambda_function.py")
            return
...
def setup_identity_pool():
    ...
    try:
        ...
        click.echo(f"2. Configure IAM policies on the authenticated role")
        click.echo(f"3. Test authentication with the client tool")
