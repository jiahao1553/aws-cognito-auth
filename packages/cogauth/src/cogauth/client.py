...
class CognitoAuthenticator:
    ...
    def _get_lambda_credentials(self, id_token, duration_hours=12, fallback_creds=None):
        ...
        # Load basic admin config for Lambda function and role names
        admin_config = self._load_basic_admin_config()
        ...
        try:
            ...
            raise Exception(
                f"Lambda function '{admin_config['aws_service_names']['lambda_function_name']}' not found. Please deploy it first using: pip install cogadmin && cogadmin lambda deploy"
            ) from None
    ...
    def _load_basic_admin_config(self):
        """Load basic admin configuration for service names"""
        default_config = {
            "aws_service_names": {
        ...
        if admin_config_file.exists():
            try:
                with open(admin_config_file) as f:
                    ...
                    if "aws_service_names" in file_config:
                        default_config["aws_service_names"].update(file_config["aws_service_names"])
            except Exception:
                pass

        # Try to load from local project config
        local_admin_config = Path("admin-config.json")
